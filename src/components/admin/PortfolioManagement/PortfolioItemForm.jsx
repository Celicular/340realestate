...
import { updatePortfolioItem } from "../../../firebase/firestore";
...
const PortfolioItemForm = ({ item, onSubmit, onCancel }) => {
  ...
  // =====================================================
  // INIT FORM FROM ITEM (LIKE RENTAL FORM)
  // =====================================================
  useEffect(() => {
    if (!item?.id) return;
  ...
  const processAndUpload = async (files) => {
    ...
    const processed = await Promise.all(
      files.map(async (file, i) => {
        ...
        return new File([compressed], `img-${Date.now()}-${i}.jpg`, {
          type: "image/jpeg",
        });
      })
    );
  ...
  const addImageUrl = async () => {
    ...
    const safeTitle = (formData.title || formData.id)
      .replace(/[^a-zA-Z0-9-_ ]/g, "")
      .trim()
      .replace(/\s+/g, "-");
    ...
    const data = await res.json();
    if (!data?.url) return alert("Image import failed");
  ...
  // =====================================================
  // SUBMIT (SESSION REFRESH ONLY HERE)
  // =====================================================
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ session refreshed, never read on load
      sessionStorage.setItem("propertyId", formData.id);
  ...
  return (
    <div className="space-y-6">
      ...
      >
        {/* ✅ ID FIELD (VISIBLE, NOT USED FOR LOADING) */}
        <div>
          <label className="block font-medium mb-1">Property ID</label>
          <input
            value={formData.id}
            readOnly
